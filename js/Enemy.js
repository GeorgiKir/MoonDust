...
class Enemy {
  ...
  constructor(theRoot, enemySpot) {
    ...
    this.domElement.src = "./images/bad-ship.png";
  ...
  enemyAnimation() {
    let myNum = 1;
    const enemyAnimInt = setInterval(() => {
      if (!myNum == 0 && this.destroyed == false) {
        this.domElement.src = "./images/bad-ship.png";
        myNum = 0;
      } else if (myNum == 0 && this.destroyed == false) {
        this.domElement.src = "./images/bad-ship2.png";
        myNum = 1;
      } else {
        clearInterval(enemyAnimInt);
      }
    }, 100);
  }
  ...
  update(timeDiff) {
    ...
    // Adding set-up for collisions
    enemyPosition = this.y + ENEMY_HEIGHT;
    if (
      enemyPosition >= GAME_HEIGHT - PLAYER_HEIGHT + 20 &&
      this.domElement.style.left == playerPosition
    ) {
      collision++;
      console.log("COLLISION");
      const heartDivSelector = document.getElementById("heartDiv");
      heartDivSelector.removeChild(heartDivSelector.lastElementChild);
      this.root.removeChild(this.domElement);
      this.destroyed = true;
    } else if (scoreCounter > 2000 && scoreCounter <= 5000) {
      this.speed = Math.random() / 2 + 0.07;
    } else if (scoreCounter > 5000 && scoreCounter <= 7000) {
      this.speed = Math.random() / 2 + 0.09;
    } else if (scoreCounter > 7000 && scoreCounter <= 11000) {
      this.speed = Math.random() / 2 + 0.1;
    } else if (scoreCounter > 11000 && scoreCounter <= 15000) {
      this.speed = Math.random() / 2 + 0.15;
    } else if (scoreCounter > 15000 && scoreCounter <= 20000) {
      this.speed = Math.random() / 2 + 0.2;
    } else if (scoreCounter > 20000) {
    ...
    // if (enemyPosition == laserYPosition) {
    //   console.log("BOOM!");
    // }
